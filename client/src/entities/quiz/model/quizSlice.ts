		...
		addScore: (state, action: PayloadAction<number>) => {
			...
			if (state.scores.length) {
				...
				while (low < high) {
					...
					if (state.scores[mid] > newScore) {
						// if current element > newScore, search right
						low = mid + 1;
					} else {
						// if current element <= newScore maybe it where we should paste element
						insertionIndex = mid;
						high = mid;
					}
				}
...
export const getQuestions = createSelector([getQuizzes], (quizzes) => quizzes.questions);
export const getQuizzesLength = (state: RootState) => state.quiz.questions?.length;
...
export const getCurrentQuizOptions = createSelector(
	[getQuestions, getCurrentQuestionId],
	(questions, currentQuestionId) => questions?.[currentQuestionId].options,
);
export const getCurrentQuizCorrectAnswer = createSelector(
	[getQuestions, getCurrentQuestionId],
	(questions, currentQuestionId) => questions?.[currentQuestionId].correct,
);
